...
import CategoriesItem from '../CategoriesItem/NavigationItem';

const Categories = ({ isFooter }) => {
  //адаптивне меню, ховаєм в кнопку нав
  let [isShowPAnel, setIisShowPAnel] = useState(false)
  const [categoties, setCategoties] = useState([]);// БД з навігацією
  ...
  return (
    ...
    <div id="addnew">
      ...
      <nav className="navigation">
        {categoties.map((element) => {
          return (
            <CategoriesItem data={element} collection={collection}
            />
          )
        })
        }
